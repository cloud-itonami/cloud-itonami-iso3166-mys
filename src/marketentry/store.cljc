(ns marketentry.store
  "SSoT for the MYS market-entry compliance actor, behind a `Store`
  ...
  `DatomicStore` uses `langchain-store.core` for its EDN-blob codec and
  event-log read/append helpers (ADR-2607141600) -- the shared seam,
  not a hand-rolled enc/dec* two-liner.
...
(defn demo-data
  ...
  `:mof-certificate-issued-date` + `:filing-as-of-date` are the two
  ground-truth date facts the flagship `mof-certificate-expired?`
  check independently recomputes over (issued + 3 years vs as-of)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems MY" :portal "ePerolehan"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-mof-certificate? true :mof-certificate-issued-date "2025-01-01"
             :filing-as-of-date "2026-07-23"
             :requires-ssm? true :ssm-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MYS" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "ePerolehan"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-mof-certificate? true :mof-certificate-issued-date "2025-01-01"
             :filing-as-of-date "2026-07-23"
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems MY" :portal "ePerolehan"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "MYS" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export MY" :portal "ePerolehan"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-mof-certificate? true :mof-certificate-issued-date "2020-01-01"
             :filing-as-of-date "2026-07-23"
             :requires-ssm? true :ssm-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MYS" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics MY" :portal "ePerolehan"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-mof-certificate? true :mof-certificate-issued-date "2025-01-01"
             :filing-as-of-date "2026-07-23"
             :requires-ssm? true :ssm-verified? false
             :drafted? false :submitted? false
             :jurisdiction "MYS" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech MY" :portal "ePerolehan"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-mof-certificate? mof-certificate-issued-date filing-as-of-date
                               requires-ssm? ssm-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-mof-certificate?)     (assoc :engagement/requires-mof-certificate? requires-mof-certificate?)
    mof-certificate-issued-date           (assoc :engagement/mof-certificate-issued-date mof-certificate-issued-date)
    filing-as-of-date                     (assoc :engagement/filing-as-of-date filing-as-of-date)
    (some? requires-ssm?)                 (assoc :engagement/requires-ssm? requires-ssm?)
    (some? ssm-verified?)                 (assoc :engagement/ssm-verified? ssm-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-mof-certificate? :engagement/mof-certificate-issued-date :engagement/filing-as-of-date
   :engagement/requires-ssm? :engagement/ssm-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-mof-certificate? (boolean (:engagement/requires-mof-certificate? m))
     :mof-certificate-issued-date (:engagement/mof-certificate-issued-date m)
     :filing-as-of-date (:engagement/filing-as-of-date m)
     :requires-ssm? (boolean (:engagement/requires-ssm? m))
     :ssm-verified? (boolean (:engagement/ssm-verified? m))
