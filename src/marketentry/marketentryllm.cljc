(ns marketentry.marketentryllm
  ...
  the MYS public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "mof-certificate-issued-date=" (:mof-certificate-issued-date e)
                        " filing-as-of-date=" (:filing-as-of-date e)
                        " ssm-verified?=" (:ssm-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ssm? e))
                              (:ssm-verified? e))
                          (or (not (:requires-mof-certificate? e))
                              (not (registry/mof-certificate-expired? e))))
                   0.9 0.3)}))
