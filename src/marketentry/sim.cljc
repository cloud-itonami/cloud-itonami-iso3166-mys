...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MYS, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up mof-certificate-expired) ==")
    ...
    (println "== filing/submit eng-4 (MOF Certificate expired -> HARD hold, flagship check) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up ssm-unverified) ==")
    ...
    (println "== filing/submit eng-5 (ssm-unverified -> HARD hold) ==")
