  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems MY"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems MY" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest mof-certificate-expired-is-held-and-unoverridable
  (testing "MOF Certificate whose 3-year validity window already lapsed by the
  declared filing-as-of-date -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:mof-certificate-expired} (-> (store/ledger db) last :basis)))
...
(deftest ssm-unverified-is-held-and-unoverridable
  (testing "unverified SSM registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:ssm-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
